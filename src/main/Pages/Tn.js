...
function Tn({ title, langToShow }) {
  ...
  let [PresentationTn, setPresentationTn] = useState(false);
  ...
  const clickOnImage = (e) => {
    ...
    setPresentationTn((PresentationTn) => true);
  ...
  const callback = () => {
    setPresentationTn((PresentationTn) => false);
  };

  return (
    <div id="Tn" className=".all">
      ...
      {langToShow === "En" ? (
        <p className="the-paragraph">
          For me, the theater is a place of fragile equilibrium, a space where
          order and disorder dance without ever being tamed. Under the
          spotlights, chaos becomes matter: it breathes, organizes itself, then
          collapses again. This series is a wandering between gesture and light.
          I seek beauty in turmoil, or rather, poetry in disorder. Chaos is not
          destruction; it is breath.
        </p>
      ) : (
        <p className="the-paragraph">
          Le théâtre est pour moi un lieu d’équilibre fragile, un espace où
          l’ordre et le désordre dansent sans jamais se dompter. Sous les
          projecteurs, le chaos devient matière : il respire, s’organise puis
          s’effondre à nouveau. Cette série est une errance entre le geste et la
          lumière. J’y cherche la beauté dans le trouble ou disons la poésie
          dans le désordre. Le chaos n’est pas destruction,  il est respiration.
          Théâtre National Wallonie Bruxelles et IHECS
        </p>
      )}
